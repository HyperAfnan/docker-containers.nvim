...
function M.refresh()
   ...
   docker.get_containers(function(containers, err)
   ...
   docker.get_images(function(images, err)
      if images == nil then
         vim.notify(err, vim.log.levels.ERROR)
         return
      end
      docker_data.images = images
   end)

   docker.get_volumes(function(volumes, err)
      if volumes == nil then
         vim.notify(err, vim.log.levels.ERROR)
         return
      end
      docker_data.volumes = volumes
   end)

   docker.get_networks(function(networks, err)
      if networks == nil then
         vim.notify(err, vim.log.levels.ERROR)
         return
      end
      docker_data.networks = networks
   end)
