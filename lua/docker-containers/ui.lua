...
local function render_tree(root)
  ...
  local function render_node(node, indent)
    ...
    elseif node.kind == "project" then
      ...
      local line = spaces .. icon .. " " .. config.icons.project .. " " .. node.data.name .. " (" .. #node.children .. ")"
    ...
    elseif node.kind == "container" then
      ...
      local status_icon = node.data.state == "running" and config.icons.container_running or config.icons.container_stopped
...
local function apply_highlights()
  ...
  for line_num, node in pairs(M.line_to_node) do
    ...
    if node.kind == "section" then
      -- Highlight icon
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.ICON, line_idx, 0, 1)
      -- Highlight section name
      local name_start = line:find(node.data.name)
      if name_start then
        vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.SECTION, line_idx, name_start - 1, name_start - 1 + #node.data.name)
      end
      -- Highlight count
      local count_start = line:find('%(')
      if count_start then
        vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.COUNT, line_idx, count_start - 1, #line)
      end
      
    elseif node.kind == "project" then
      -- Apply project highlight to entire line first (as base)
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.PROJECT, line_idx, 0, #line)
      ...
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.ICON, line_idx, spaces_len, icon_end)
      
    elseif node.kind == "container" then
      -- Highlight container name
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.CONTAINER, line_idx, 0, #line)
      -- Highlight status indicator
      local leading_spaces = line:match("^(%s*)")
      local spaces_len = #leading_spaces
      local status_icon_end = spaces_len + 3  -- Status icons are 3 bytes in UTF-8
      
      local status_hl = node.data.state == "running" and highlights.STATUS_RUNNING or highlights.STATUS_STOPPED
      
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, status_hl, line_idx, spaces_len, status_icon_end)
      
    elseif node.kind == "image" then
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.IMAGE, line_idx, 0, #line)
      
    elseif node.kind == "volume" then
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.VOLUME, line_idx, 0, #line)
      
    elseif node.kind == "network" then
      vim.api.nvim_buf_add_highlight(M.sidebar_buf, highlights.ns_id, highlights.NETWORK, line_idx, 0, #line)
    end
...
local function stop_container()
  ...
  docker.stop_container_async(container_name, function(success, message)
    if success then
...
local function restart_container()
  ...
  docker.restart_container_async(container_name, function(success, message)
    if success then
